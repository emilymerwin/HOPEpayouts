...
# Open the CSV  
f = open( 'HOPEpayouts - Sheet2.csv', 'rU' )  
...
for i, row in enumerate(reader):
	...
	try:
		...
		elementArr[holder]['school']['years'] = {}
		#elementArr[holder]['school']['holder'] = holder
		#print subtree
		#print "except"
	
	elementArr[holder]['school']['years']["fy" + row['fiscyear']] = {}
	#elementArr[holder]['school']['fiscyear']['year'] = row['fiscyear']
	elementArr[holder]['school']['years']["fy" + row['fiscyear']]['hopepays'] = round(float(row['hopepays']))
	elementArr[holder]['school']['years']["fy" + row['fiscyear']]['tuitionfees'] = round(float(row['tuitionfees']))
	elementArr[holder]['school']['years']["fy" + row['fiscyear']]['studentowes'] = round(float(row['studentowes']))
